...
fn check<E: Workload>(dir: &tempfile::TempDir, name: &str) {
    ...
    for id in &range_ids {
        ...
        assert!(
            a.rating >= 3 && a.rating <= 5,
            "{name}: range_query returned an out-of-range asset"
        );
        assert!(
            a.iso >= 100 && a.iso <= 3200,
            "{name}: range_query returned an out-of-range asset"
        );
    }
    ...
    assert_eq!(
        range_ids.len(),
        expected_range,
        "{name}: range_query count mismatch"
    );
    ...
    assert_eq!(
        kw_ids, expected_kw,
        "{name}: keyword_subtree_query mismatch"
    );

    engine.write_rating(assets[0].id, 5).unwrap();
    let facets = engine
        .faceted_filter(None, Some(5), Some(BENCH_LEAF_KEYWORD))
        .unwrap();
    assert!(
    ...
    assert_eq!(
        page.len(),
        20,
        "{name}: sort_by_date_page returned the wrong page size"
    );
    ...
    assert_eq!(
        dates, sorted_desc,
        "{name}: sort_by_date_page is not sorted newest-first"
    );

    let some_folder = &assets[0].folder_path;
    let folder_count = engine.folder_subtree_count(some_folder).unwrap();
    let expected_folder_count = assets
        .iter()
        .filter(|a| a.folder_path.starts_with(some_folder.as_str()))
        .count() as u64;
    assert_eq!(
        folder_count, expected_folder_count,
        "{name}: folder_subtree_count mismatch"
    );
    ...
    let expected_name: Vec<u64> = {
        let mut v: Vec<u64> = assets
            .iter()
            .filter(|a| a.filename.contains("00042"))
            .map(|a| a.id)
            .collect();
        v.sort_unstable();
        v
    ...
    assert_eq!(
        name_ids_sorted, expected_name,
        "{name}: filename_search mismatch"
    );

    engine
        .tag_keyword(&[assets[1].id], "Cross.Engine.Test")
        .unwrap();
    let tagged = engine.keyword_subtree_query("Cross.Engine.Test").unwrap();
    assert_eq!(
        tagged,
        vec![assets[1].id],
        "{name}: tag_keyword + keyword_subtree_query mismatch"
    );

    assert!(
        engine.integrity_check().unwrap(),
        "{name}: integrity_check failed on a fresh store"
    );
}
