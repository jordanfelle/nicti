...
schema-fit reconsideration, and #113/ADR-0014's `libSQL` follow-up — one module per candidate,
...
and `libsql` are all default-off, evaluated-not-adopted, kept for reference — `libsql` additionally
cannot be enabled in the same binary as `sqlite`, both bundle their own SQLite C symbols and
collide at link time, see ADR-0014's Spike section; the two facet-cache modules require `sqlite`,
and `facet_cache_duckdb` additionally requires `duckdb`), plus `schema_fit.rs` (ADR-0002's
